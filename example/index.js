...
(() => {
    ...
    function installDefaultPlugins() {
        ...
        window.localStorage.removeItem('mct-recent-objects');
        openmct.router.once('change:path', () => {
            const eventHandlers = openmct.router._events['change:path'];
            let recentObjectsPathChangeEventHandler;

            if (eventHandlers instanceof Array) {
                recentObjectsPathChangeEventHandler = eventHandlers.filter((handler) => handler.fn.name === 'bound onPathChange')[0];
            } else if (eventHandlers?.fn?.name === 'bound onPathChange') {
                recentObjectsPathChangeEventHandler = eventHandlers;
            }

            if (recentObjectsPathChangeEventHandler !== undefined) {
                openmct.router.off('change:path', recentObjectsPathChangeEventHandler.fn);
                recentObjectsPathChangeEventHandler.fn = () => {};
            }
        });
