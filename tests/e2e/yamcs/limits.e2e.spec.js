...
test.describe("Mdb runtime limits tests @yamcs", () => {
    ...
    test.beforeEach(async ({page}) => {
        ...
        await assertLimitsAreCleared(page);
        ...
        await page
            .getByRole('list', { name: 'Plot Series Properties' })
            .getByTitle('Display limit lines')
            .getByRole('input')
            .check();
    ...
    test('Can show mdb limits when enabled', async ({ page }) => {
        // Ensure that the changed limits are now displayed without a reload
        await assertLimitLinesExistAndAreVisible(page, 'watch');
    });

    test('Can show changed mdb limits when you navigate to the view', async ({ page }) => {
        ...
        // Ensure that the changed limits are now displayed without a reload
        await assertLimitLinesExistAndAreVisible(page, 'critical');
    });

    test('No new requests for data are made when limits are redrawn due to window resizing', async({ page }) => {
        // Resize the chart container by showing the snapshot pane.
        await page.getByRole('button', { name: 'Show' }).click();

        // No request was made to get historical data
        const resizeAndRedrawLimitsRequests = [];
        page.on('request', (req) => {
            resizeAndRedrawLimitsRequests.push(req);
        });
        expect(resizeAndRedrawLimitsRequests.length).toEqual(0);
    });
});
 ...
 */
async function assertLimitLinesExistAndAreVisible(page, level) {
    // Wait for plot series data to load
    await waitForPlotsToRender(page);
    // Wait for limit lines to be created
    await page.waitForSelector('.c-plot-limit-line', { state: 'attached' });
    //c-plot-limit-line--critical, c-plot-limit-line--watch
    const limitLineCount = await page.locator(`.c-plot-limit-line.c-plot-limit-line--${level}`).count();
    // There should be 2 limit lines created by default
    expect(await page.locator(`.c-plot-limit-line.c-plot-limit-line--${level}`).count()).toBe(2);
    for (let i = 0; i < limitLineCount; i++) {
        await expect(page.locator(`.c-plot-limit-line.c-plot-limit-line--${level}`).nth(i)).toBeVisible();
    }
}

/**
 * Asserts that limits are cleared
 * @param {import('@playwright/test').Page} page
 */
async function assertLimitsAreCleared(page) {
    const runTimeLimitChangeResponse = await page.request.get(`${YAMCS_URL}api/mdb-overrides/myproject/realtime/parameters/myproject/Detector_Temp`);
    await expect(runTimeLimitChangeResponse).toBeOK();
    await expect(await runTimeLimitChangeResponse.json()).toEqual({});
}
